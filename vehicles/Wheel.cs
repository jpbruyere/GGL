...
namespace JitterExtensions
{
    ...
    */
    ...
    public class Wheel
    {
        ...
        public Wheel(Jitter.World world, RigidBody car, JVector position, float radius)
        {
            this.world = world;
            this.car = car;
        ...
        {
            return car.Position +
                JVector.Transform(Position + JVector.Backward * displacement, car.Orientation);
        }
        ...
        public void PostStep(float timeStep)
        {
            ...
            //upSpeed /= 1000f;
            //Debug.WriteLine("displacement:{0} lastdisp:{1} upSpeed:{2} timestep:{3}", displacement, lastDisplacement, upSpeed, timeStep);
            if (Locked)
            {
                angVel = 0;
                torque = 0;
        ...
        public void PreStep(float timeStep)
        {
            ...
            JVector worldAxis = JVector.Transform(new JVector(0, 0, 1), car.Orientation);
            JVector worldPos = car.Position + JVector.Transform(Position, car.Orientation);

            JVector forward = new JVector(-car.Orientation.M21, -car.Orientation.M22, -car.Orientation.M23 );
            //forward = JVector.Cross(forward, new JVector(1,0,0));


            JVector wheelFwd = JVector.Transform(forward, JMatrix.CreateFromAxisAngle(new JVector(0, 0, 1), SteerAngle / 360 * 2 * JMath.Pi));
            JVector wheelLeft = JVector.Cross(new JVector(0, 0, 1), wheelFwd); wheelLeft.Normalize();
            ...
            // add force to car
            car.AddForce(force, groundPos + 0.5f * new JVector(0,0,1));
            ...
            //debug
            grndNormal = Conversion.jToOtk(groundNormal);
            grndPos = Conversion.jToOtk(groundPos);
            grndForward = Conversion.jToOtk(groundFwd);
            grndUp = Conversion.jToOtk(groundUp);

            wLeft = Conversion.jToOtk(wheelLeft);
            wFwd = Conversion.jToOtk(wheelFwd);
            wUp = Conversion.jToOtk(wheelUp);

        }

        public Vector3 wFwd = Vector3.Zero;
        public Vector3 wLeft = Vector3.Zero;
        public Vector3 wUp = Vector3.Zero;

        public Vector3 grndNormal = Vector3.Zero;
        public Vector3 grndUp = Vector3.Zero;
        public Vector3 grndForward = Vector3.Zero;
        public Vector3 grndPos = Vector3.Zero;
