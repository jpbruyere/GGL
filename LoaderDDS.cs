...
namespace TextureLoaders
{
    ...
    static class ImageDDS
    {
        ...
        private static int  _Depth, _MipMapCount;
		...
		public static void LoadFromByteArray(byte[] _RawDataFromFile, out GGL.Texture texture, string filename = "")
		{
			#region Prep data
			// invalidate whatever it was before
			texture = new GGL.Texture();
			texture.TexTarget = (TextureTarget) 0;
			texture.texRef = TextureLoaderParameters.OpenGLDefaultTexture;
			...
			{
				texture.TexTarget = TextureTarget.Texture3D; // image is 3D Volume
				_Depth = (int) dwDepth;
				throw Unfinished;
			} else
			{// image is 2D or Cube
				...
				{
					texture.TexTarget = TextureTarget.TextureCubeMap;
					_Depth = 6;
				} else
				{
					texture.TexTarget = TextureTarget.Texture2D;
					_Depth = 1;
				}
			}
			...
			if ( TextureLoaderParameters.Verbose )
				Trace.WriteLine( "\n" + GetDescriptionFromMemory( filename, texture ) );
			#endregion Translate Header to less cryptic representation

			#region send the Texture to GL
			#region Generate and Bind Handle
			GL.GenTextures( 1, out texture.texRef );
			GL.BindTexture( texture.TexTarget, texture.texRef );
			...
			for ( int Slices = 0 ; Slices < _Depth ; Slices++ )
			{
				...
				for ( int Level = 0 ; Level < _MipMapCount ; Level++ ) // start at base image
				{
					...
					{ // Note: there could be a potential problem with non-power-of-two cube maps
						...
						default:
							throw new ArgumentException( "ERROR: Use DXT for 2D Images only. Cannot evaluate " + texture.TexTarget );
						}
						GL.Finish( );
						...
						case TextureTarget.Texture3D:
							GL.GetTexLevelParameter( texture.TexTarget, Level, GetTextureParameter.TextureWidth, out width );
							GL.GetTexLevelParameter( texture.TexTarget, Level, GetTextureParameter.TextureHeight, out height );
							GL.GetTexLevelParameter( texture.TexTarget, Level, GetTextureParameter.TextureInternalFormat, out internalformat );
							GL.GetTexLevelParameter( texture.TexTarget, Level, GetTextureParameter.TextureCompressed, out compressed );
							break;
						case TextureTarget.TextureCubeMap:
						...
						{
							GL.DeleteTextures( 1, ref texture.texRef );
				...
				GL.TexParameter( texture.TexTarget, (TextureParameterName) All.TextureBaseLevel, 0 );
				GL.TexParameter( texture.TexTarget, (TextureParameterName) All.TextureMaxLevel, trueMipMapCount );

				int TexMaxLevel;
				GL.GetTexParameter( texture.TexTarget, GetTextureParameter.TextureMaxLevel, out TexMaxLevel );
			...
			GL.TexParameter( texture.TexTarget, TextureParameterName.TextureMinFilter, (int) TextureLoaderParameters.MinificationFilter );
			GL.TexParameter( texture.TexTarget, TextureParameterName.TextureMagFilter, (int) TextureLoaderParameters.MagnificationFilter );

			GL.TexParameter( texture.TexTarget, TextureParameterName.TextureWrapS, (int) TextureLoaderParameters.WrapModeS );
			GL.TexParameter( texture.TexTarget, TextureParameterName.TextureWrapT, (int) TextureLoaderParameters.WrapModeT );
		...
		public static void LoadFromDisk( string filename, out GGL.Texture texture )
        {
            ...
            try // Exceptions will be thrown if any Problem occurs while working on the file. 
            {
				...
				LoadFromByteArray(_RawDataFromFile, out texture, filename);
			}catch (Exception e){
 ...
 "\n\n---Capabilities---" + filename +
		...
		private static string GetDescriptionFromMemory( string filename, GGL.Texture texture )
        {
            return "\nFile: " + filename +
					"\nDimension: " + texture.TexTarget +
                   "\nSize: " + texture.Width + " * " + texture.Height + " * " + _Depth +
