...
void main( void ) {
	...
	vec2 position = texCoord - mouse;
	position.y *= resolution.y/resolution.x;
	float ang = atan(position.y, position.x);
	float dist = length(position);
	out_frag_color.rgb = vec3(0.3, 0.5, 0.7) * (pow(dist, -1.0) * 0.05);
	for (float ray = 0.0; ray < 60.0; ray += 1.0) {
		...
		brite -= dist * 0.2;
		if (brite > 0.0) {
			out_frag_color.rgb += vec3(0.2+0.4*rand(8644, ray), 0.4+0.4*rand(4567, ray), 0.5+0.4*rand(7354, ray)) * brite;
		}
	}
