...
namespace GGL
{
    ...
    public class Terrain
    {
        ...
        public Terrain(float[,] _heights, int _x = 0, int _y = 0)
        {
            Heights = _heights;
            xWorld = _x;
            yWorld = _y;
        ...
        {
            LOD = (int)(distFrom(World.vEye) / Terrain.size);
        }
        ...
        public void render()
        {
            ...
            GL.PushMatrix();
            GL.Translate(xWorld * Terrain.size * Terrain.sizeScale, yWorld * Terrain.size * Terrain.sizeScale, 0f);
    ...
    public class TerrainModel : Model
    {
        ...
        public int texture
        {
            get { return meshe.Faces[0].texture; }
            set
            {
                foreach (FaceGroup fg in meshe.Faces)
                {
                    fg.texture = value;
                }
            }
        ...
        public static void RemoveDuplicateVertices(List<TriangleVertexIndices> indices,
        ...
        {
            List<JVector> positions = meshe.Vertices.Select(v => new JVector(v.position.X, v.position.Y, v.position.Z)).ToList();
                ...
                while (x < Terrain.size)
                {
                    indices.Add(
                        new TriangleVertexIndices
                            (
                                x + y * (Terrain.size + 1),
                                x + (y + 1) * (Terrain.size + 1),
                                x + 1 + y * (Terrain.size + 1)
                            )
                    );
        ...
        public override void Render()
        {
            ...
            if (renderCollisionNormals)
                drawCollisionNormals();
        }
        public override Vector3[] collisionNormals
        {
            get
            {
                if (_collisionNormals == null)
                {
                    TriangleMeshShape tms = shape as TriangleMeshShape;

                    List<Vector3> cN = new List<Vector3>();

                    foreach (Vertex v in meshe.Vertices)
                    {
                        JVector result = new JVector(v.position.X, v.position.Y, v.position.Z);
                        tms.CollisionNormal(out result);
                        cN.Add(new Vector3(result.X, result.Y, result.Z));
                    }
                    _collisionNormals = cN.ToArray();
                }

                return _collisionNormals;
            }
        }
        public override void drawCollisionNormals()
        {
            if (renderCollisionNormals)
            {
                GL.Disable(EnableCap.Lighting);
                ...
                for (int i = 0; i < meshe.Vertices.Length; i++)
                {
                    GL.Vertex3(meshe.Vertices[i].position);
                    GL.Vertex3(collisionNormals[i]);
                }
