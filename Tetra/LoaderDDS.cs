...
namespace TextureLoaders
{
    ...
    static class ImageDDS
    {
		...
		public static void LoadFromByteArray(byte[] _RawDataFromFile, out Tetra.Texture texture, string filename = "")
		{
			#region Prep data
			// invalidate whatever it was before
			texture = new Tetra.Texture();
			...
			for ( int Slices = 0 ; Slices < _Depth ; Slices++ )
			{
				...
				for ( int Level = 0 ; Level < _MipMapCount ; Level++ ) // start at base image
				{
					...
					if ( Level == 0 && _IsCompressed && _BytesForMainSurface != SurfaceSizeInBytes )
			...
			GL.TexParameter( texture.TexTarget, TextureParameterName.TextureMinFilter, (int) Tetra.Texture.DefaultMinFilter );
			GL.TexParameter( texture.TexTarget, TextureParameterName.TextureMagFilter, (int) Tetra.Texture.DefaultMagFilter );

			GL.TexParameter( texture.TexTarget, TextureParameterName.TextureWrapS, (int)  Tetra.Texture.DefaultWrapMode );
			GL.TexParameter( texture.TexTarget, TextureParameterName.TextureWrapT, (int)  Tetra.Texture.DefaultWrapMode );
		...
		public static void LoadFromDisk( string filename, out Tetra.Texture texture )
        {
 ...
 "\n\n---Capabilities---" + filename +
		...
		private static string GetDescriptionFromMemory( string filename, Tetra.Texture texture )
        {
            return "\nFile: " + filename +
